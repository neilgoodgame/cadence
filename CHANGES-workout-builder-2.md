...
## 4. New screen: Workout Library

`Workout Library.dc.html` — a dedicated browse/search screen for saved workouts,
separate from the Builder. Sidebar "Workouts" nav item now links here (was a
non-interactive label); "+ New workout" in its top bar opens the Builder.

- **Organization**: folders (single-membership, e.g. "VO2 Max", "Threshold",
  "Base / Endurance", "Race Prep") in a left rail, plus free-form multi-value
  tags (filter by clicking a tag chip) and a sport filter (All/Bike/Run).
- **Search & sort**: text search on name; sort by recently updated, name,
  duration, TSS, or most-used.
- **Grid or list view** toggle. Grid cards and list rows both show: sport
  badge, name, duration (hh:mm:ss), TSS, step count, tags, and a mini
  interval-chart preview (bars from the flattened/unrolled step tree, colored
  by sport). Row/card actions: assign, edit (→ Builder), duplicate, export,
  delete.
- **Multi-select** (checkbox per card/row) surfaces a bulk action bar: add tag,
  export, delete, clear selection.
- **Assign modal**: athlete chips + date, matching the Builder's publish flow.

## 5. Builder → Library save flow

- Workout Designer's "Save draft" button is renamed **"Save"** and now
  actually persists: it serializes the current step tree (name, sport, total
  duration, estimated TSS, step count, a chart-preview array) into a shared
  `cadence_library_workouts` store, then navigates to the Workout Library,
  where the saved workout appears immediately.
- **Prototype-only mechanism**: the two `.dc.html` files have no shared
  backend, so the handoff prototype uses `localStorage` as a stand-in. In the
  real app this is just "create/update workout via the API" — implement
  `POST /v1/workouts` (or `PATCH` on subsequent saves) from the Builder, and
  have the Library screen list workouts from `GET /v1/workouts` instead of any
  client-side store.
...
7. Build the Workout Library screen (folders, tags, sport filter, search/sort,
   grid/list, bulk actions, assign modal) backed by `GET /v1/workouts`
   (filter/sort params as needed) rather than local state.
8. Wire the Builder's Save action to `POST`/`PATCH /v1/workouts`, then route
   to the Library on success (replace the prototype's `localStorage` shim).
